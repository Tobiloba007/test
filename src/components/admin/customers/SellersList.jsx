...
const SellersList = ({setOpenModal, setData, setCustomerDetails}) => {
      ...
      const totalPages = Math.ceil(sellersData.length / itemsPerPage); // Calculate total pages
      ...
      const currentPageItems = sellersData.slice(startIndex, endIndex);
      ...
      useEffect(() => {
        dispatch(sellerListAction(setSellersData, setLoading, setError))

        if (searchTerm !== '') {
            dispatch(searchSeller(setSearchResults, searchTerm))
            setSearch(true)
          } else {
            setSearchResults([]);
            setSearch(false)
      ...
      const handleOpenDetails = (item) => {
         dispatch(sellerCustomerDetails(item, setCustomerDetails))
      }
     ...
     ?<div className='relative flex flex-col items-center justify-start w-full h-[29.5rem] border-[1px] border-[#dddddd] rounded-lg mt-3 mb-12 py-2 pb-5 md:h-[27.5rem] 
             ...
             ?<tbody className='w-full'>
                  ...
                  <tr key={item.id} onClick={()=>handleOpenDetails(item.id)} className='h-14 hover:bg-[#EEEEEE]'>
                      ...
                      <td class="border-b text-[10px] font-medium">{item.supply_request_count}</td>
            ...
            :<tbody className='w-full'>
               ...
               {/* PAGINATION */}
               <div className='absolute bottom-5 flex flex-row iems-center justify-between w-full bg-white pt-3 px-5 mt-3'>
